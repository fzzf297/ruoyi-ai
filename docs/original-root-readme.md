...
在 RuoYi 管理后台里嵌入一个 **AI 助手面板**。用户通过右下角悬浮按钮打开右侧对话栏，也可以切换为可拖拽浮层；支持文字输入与语音输入。用户说「查一下张三最近登录记录」「打开用户管理」「把当前选中的用户停用」，助手理解意图后调用既有业务接口完成操作，而不是另建一套系统、也不直连数据库。
...
- **输入形态**：文字输入与语音输入统一进入 Agent 链路；语音先转写为文本，再复用同一套工具、确认与审计机制
...
| [前端技术文档](docs/frontend/README.md) | 悬浮按钮、右侧栏/浮层、拖拽、语音输入、SSE、事件渲染 |
| [后端技术文档](docs/backend/README.md) | RuoYi Java 支撑、Agent 服务、语音转写、工具、实体解析、确认与幂等 |
